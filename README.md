...
- Анализируются только сообщения-литералы (```"..."```) в первом аргументе.
- Если проект не компилируется/есть ошибки модулей, ```go/analysis``` линтеры могут не запускаться или давать неполный результат.
